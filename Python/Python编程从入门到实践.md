...
## 6.1 访问字典中的值
在Python中，字典是一系列**键-值对**。每个键对应一个值，可以使用键来访问与之相关联的值。"值"可以是数字、字符串、列表乃至字典。事实上，可将任何Python对象用作字典中的值。

字典用放在花括号{}中的一系列键-值对表示。键和值之间用冒号分隔，键-值对之间用逗号分隔。

```py
alien_0 = {'color': 'green', 'points': '5'}
```
### 6.1.1 访问字典中的值
要获取"值"，可依次指定字典名和放在方括号内的键。
```py
alien_0 = {'color': 'green', 'points': '5'}
print(alien_0['color'])
...
```
### 6.1.2 添加键-值对
**注意，键-值对的排列顺序与添加顺序不同。Python不关心键-值对的添加顺序，只关心键和值之间的关联关心。**
```py
alien_0 = {'color': 'green', 'points': '5'}
print(alien_0)
...
嵌套函数特点：
1. 可以访问外部函数变量(指的是外部函数内、内部函数外的变量) ;
2. 内部函数可以修改外部函数的可变变量，如list ;
3. 内部函数若要修改外部函数不可变变量，需要在内部函数声明变量，nonlocal 变量名 ;
4. 内部函数若要修改全局变量，需要在内部函数声明变量，global 变量名;
5. locals() 查看声明的本地(内部)变量（包括函数）有哪些，以字典的形式输出，key为变量名，value为变量值;

   globals() 查看声明的全局变量（包括函数）有哪些，以字典的形式输出，会有一些系统的键值对。

## 闭包
条件格式：
1. 外部函数中定义了内部函数;
2. 内部函数引用了外部函数变量;
3. 外部函数有返回值, 且返回值是内部函数名。
